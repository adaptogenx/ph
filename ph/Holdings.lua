...
function pH_Holdings:AddLot(session, itemID, count, expectedEach, bucket)
    if not session or not session.holdings then
        error("pH_Holdings:AddLot - Invalid session or missing holdings")
    end
    ...
    if pH_DB_Account.debug.verbose then
        print(string.format("[pH Holdings] Added lot: itemID=%d, count=%d, expectedEach=%d, bucket=%s",
            itemID, count, expectedEach, bucket))
    end
end

-- Get total count for an item in holdings
function pH_Holdings:GetCount(session, itemID)
...
function pH_Holdings:GetExpectedValue(session, itemID)
    if not session or not session.holdings or not session.holdings[itemID] then
        return 0
    end
...
function pH_Holdings:GetTotalExpectedValue(session)
    if not session or not session.holdings then
        return 0
    end
...
function pH_Holdings:ConsumeFIFO(session, itemID, count)
    if not session or not session.holdings or not session.holdings[itemID] then
    ...
    if pH_DB_Account.debug.verbose then
        for bucket, value in pairs(bucketValues) do
            print(string.format("[pH Holdings] Consumed FIFO: itemID=%d, bucket=%s, value=%d",
                itemID, bucket, value))
        end
    end
