--[[
    Ledger.lua - Double-entry bookkeeping for pH
...
function pH_Ledger:InitializeLedger(session)
    session.ledger = {
        balances = {
            -- Assets
            ["Assets:Cash"] = 0,
...
function pH_Ledger:Post(session, debitAccount, creditAccount, amountCopper, meta)
    if not session or not session.ledger then
        error("pH_Ledger:Post - Invalid session")
    end

    if amountCopper < 0 then
        error("pH_Ledger:Post - Amount must be positive: " .. amountCopper)
    end
...
function pH_Ledger:AddBalance(session, account, deltaCopper)
    if not session or not session.ledger then
        error("pH_Ledger:AddBalance - Invalid session")
    end
...
function pH_Ledger:GetBalancesMatching(session, pattern)
    local result = {}
    if not session or not session.ledger then
        return result
    end
...
function pH_Ledger:FormatMoney(copper)
...
function pH_Ledger:FormatMoneyShort(copper)
    if not copper or copper == 0 then
        return "0g 00s"
    end
