--[[
    UI_History_Detail.lua - Detail pane with tabs for pH History
...
-- Initialize
--------------------------------------------------
function pH_History_Detail:Initialize(parent, historyController)
    self.parent = parent
    ...
    for i, tabInfo in ipairs(tabNames) do
        ...
        tab:SetScript("OnClick", function()
            pH_History_Detail:SwitchTab(tabInfo.key)
        end)
...
-- Switch Tab
--------------------------------------------------
function pH_History_Detail:SwitchTab(tabKey)
    ...
    if pH_Settings then
        pH_Settings.historyActiveTab = tabKey
    end
end
...
--------------------------------------------------
function pH_History_Detail:SetSession(sessionId)
    self.currentSessionId = sessionId
    self.currentSession = pH_SessionManager:GetSession(sessionId)
    self.currentMetrics = self.currentSession and pH_SessionManager:GetMetrics(self.currentSession) or nil
...
-- Render Active Tab
--------------------------------------------------
function pH_History_Detail:RenderActiveTab()
...
-- Render Empty State
--------------------------------------------------
function pH_History_Detail:RenderEmptyState()
...
local function FormatMetricRate(metricKey, rate)
    if metricKey == "gold" then
        return pH_Ledger:FormatMoneyShort(rate) .. "/hr"
    end
...
local function FormatMetricTotal(metricKey, total)
    if metricKey == "gold" then
        return pH_Ledger:FormatMoney(total)
    end
...
-- Render Summary Tab
--------------------------------------------------
function pH_History_Detail:RenderSummaryTab()
    ...
    local cfg = pH_Settings and pH_Settings.metricCards or {}
            ...
            for _, cat in ipairs(categories) do
                ...
                if value > 0 then
                    ...
                    AddFocusBreakdownRow(panel, rowY, label,
                        pH_Ledger:FormatMoneyShort(perHour) .. "/hr",
                        pH_Ledger:FormatMoney(value),
                        string.format("%d%%", pct))
    ...
    AddRow("Total Per Hour", pH_Ledger:FormatMoneyShort(metrics.totalPerHour) .. "/hr", pH_Colors.TEXT_MUTED)
    AddRow("Gold Per Hour", pH_Ledger:FormatMoneyShort(metrics.cashPerHour) .. "/hr", pH_Colors.ACCENT_GOOD)
    AddRow("Expected Per Hour", pH_Ledger:FormatMoneyShort(metrics.expectedPerHour) .. "/hr", pH_Colors.TEXT_MUTED)

    yOffset = yOffset - 10

    -- Gold flow (income / expenses)
    AddHeader("Income / Expenses")
    AddRow("Looted Coin", pH_Ledger:FormatMoney(metrics.income))
    AddRow("Quest Rewards", pH_Ledger:FormatMoney(metrics.incomeQuest))
    AddRow("Vendor Sales", pH_Ledger:FormatMoney(pH_Ledger:GetBalance(session, "Income:VendorSales")))
    -- Expenses: only show negative sign and red color if value is non-zero
    local repairsText = metrics.expenseRepairs > 0 and ("-" .. pH_Ledger:FormatMoney(metrics.expenseRepairs)) or pH_Ledger:FormatMoney(metrics.expenseRepairs)
    AddRow("Repairs", repairsText, metrics.expenseRepairs > 0 and pH_Colors.ACCENT_BAD or nil)
    local vendorBuysText = metrics.expenseVendorBuys > 0 and ("-" .. pH_Ledger:FormatMoney(metrics.expenseVendorBuys)) or pH_Ledger:FormatMoney(metrics.expenseVendorBuys)
    AddRow("Vendor Purchases", vendorBuysText, metrics.expenseVendorBuys > 0 and pH_Colors.ACCENT_BAD or nil)
    local travelText = metrics.expenseTravel > 0 and ("-" .. pH_Ledger:FormatMoney(metrics.expenseTravel)) or pH_Ledger:FormatMoney(metrics.expenseTravel)
    ...
    if session.pickpocket and (metrics.pickpocketGold > 0 or metrics.pickpocketValue > 0) then
        ...
        AddRow("Coin", pH_Ledger:FormatMoney(metrics.pickpocketGold))
        AddRow("Items Value", pH_Ledger:FormatMoney(metrics.pickpocketValue))
        ...
        if metrics.fromLockboxGold > 0 or metrics.fromLockboxValue > 0 then
            AddRow("From Lockbox Coin", pH_Ledger:FormatMoney(metrics.fromLockboxGold))
            AddRow("From Lockbox Items", pH_Ledger:FormatMoney(metrics.fromLockboxValue))
        end
    end
...
-- Render Items Tab
--------------------------------------------------
function pH_History_Detail:RenderItemsTab()
    ...
    for _, itemData in ipairs(itemsArray) do
        ...
        valueText:SetText(pH_Ledger:FormatMoneyShort(itemData.expectedTotal))
    ...
    summaryText:SetText(string.format("Total: %d items worth %s",
        totalItems,
        pH_Ledger:FormatMoney(totalValue)))
...
--------------------------------------------------
function pH_History_Detail:RenderGatheringTab()
...
-- Render Compare Tab
--------------------------------------------------
function pH_History_Detail:RenderCompareTab()
    ...
    AddComparisonRow(
        "Total g/hr",
        pH_Ledger:FormatMoneyShort(metrics.totalPerHour),
        pH_Ledger:FormatMoneyShort(zoneStats.avgTotalPerHour),
        nil
    )
    ...
    AddComparisonRow(
        "Expected g/hr",
        pH_Ledger:FormatMoneyShort(metrics.expectedPerHour),
    ...
    for _, otherSessionId in ipairs(allZoneSessions) do
        if otherSessionId ~= session.id then
            local otherSummary = pH_Index:GetSummary(otherSessionId)
    ...
    if zoneStats.bestSessionId and zoneStats.bestSessionId ~= session.id then
        ...
        bestInsight:SetText(string.format("* Best session in zone: %s/hr (Session #%d)",
            pH_Ledger:FormatMoneyShort(zoneStats.bestTotalPerHour),
...
-- Export module
_G.pH_History_Detail = pH_History_Detail
