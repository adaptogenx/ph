--[[
    ...
    - Manual overrides (pH_DB_Account.priceOverrides)
...
local function GetManualOverride(itemID)
    if pH_DB_Account.priceOverrides and pH_DB_Account.priceOverrides[itemID] then
        return pH_DB_Account.priceOverrides[itemID]
    end
    return nil
end
...
function pH_PriceSources:GetAHPrice(itemID)
    -- Priority 1: Manual overrides (highest priority)
    local price = GetManualOverride(itemID)
    if price then
        if pH_DB_Account.debug.verbose then
            print(string.format("[pH Price] Manual override: itemID=%d, price=%d", itemID, price))
        end
        return price
    end
    ...
    if price then
        if pH_DB_Account.debug.verbose then
            print(string.format("[pH Price] Custom AH: itemID=%d, price=%d", itemID, price))
        end
        return price
    end
    ...
    if price then
        if pH_DB_Account.debug.verbose then
            print(string.format("[pH Price] TSM: itemID=%d, price=%d", itemID, price))
        end
        return price
    end
...
function pH_PriceSources:GetAvailableSources()
    local sources = {}

    if pH_DB_Account.priceOverrides and next(pH_DB_Account.priceOverrides) ~= nil then
...
-- Export module
_G.pH_PriceSources = pH_PriceSources
