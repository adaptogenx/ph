--[[
    Debug.lua - Debug and testing infrastructure for pH
...
function pH_Debug:ValidateNetWorth(session)
    ...
    local equity = pH_Ledger:GetBalance(session, "Equity:InventoryRealization")
...
function pH_Debug:ValidateLedgerBalance(session)
    if not session or not session.ledger then
        return false, "Invalid session"
    end
...
function pH_Debug:ValidateHoldings(session)
...
function pH_Debug:ValidateInvariants(session)
    local results = {}
    local allPass = true
    ...
    if pH_DB_Account.debug.verbose or not allPass then
        for _, result in ipairs(results) do
            local color = result.ok and COLOR_GREEN or COLOR_RED
            print(string.format("%s[pH Invariant] %s: %s%s", color, result.name, result.message, COLOR_RESET))
        end
    end
...
function pH_Debug:RunTests()
    print(COLOR_YELLOW .. "[pH Test Suite] Running tests..." .. COLOR_RESET)
    ...
    print(string.format("%s[pH Test Suite] %d passed, %d failed%s",
...
function pH_Debug:Test_BasicLoot()
    local testName = "Basic Loot Posting"

    -- Start temporary session
    if not pH_SessionManager:GetActiveSession() then
        pH_SessionManager:StartSession()
    end

    local session = pH_SessionManager:GetActiveSession()
    local initialCash = pH_Ledger:GetBalance(session, "Assets:Cash")
    local initialIncome = pH_Ledger:GetBalance(session, "Income:LootedCoin")

    -- Inject loot
    local lootAmount = 500
    pH_Events:InjectLootedCoin(lootAmount)

    -- Verify balances
    local finalCash = pH_Ledger:GetBalance(session, "Assets:Cash")
    local finalIncome = pH_Ledger:GetBalance(session, "Income:LootedCoin")
...
function pH_Debug:Test_MultipleLoot()
    ...
    pH_Events:InjectLootedCoin(100)
    pH_Events:InjectLootedCoin(250)
    pH_Events:InjectLootedCoin(75)
...
function pH_Debug:Test_ZeroLoot()
    ...
    local ok = pH_Events:InjectLootedCoin(0)
...
function pH_Debug:Test_BasicRepair()
    ...
    local initialCash = pH_Ledger:GetBalance(session, "Assets:Cash")
    local initialRepairs = pH_Ledger:GetBalance(session, "Expense:Repairs")

    -- Inject repair
    local repairCost = 250
    pH_Events:InjectRepair(repairCost)

    -- Verify balances
    local finalCash = pH_Ledger:GetBalance(session, "Assets:Cash")
    local finalRepairs = pH_Ledger:GetBalance(session, "Expense:Repairs")
...
function pH_Debug:Test_NetCash()
    ...
    pH_Events:InjectLootedCoin(1000)
    pH_Events:InjectRepair(300)
...
function pH_Debug:Test_VendorSale_NoDoubleCount()
    ...
    local initialNetWorth = pH_Ledger:GetBalance(session, "Assets:Cash") +
                            pH_Holdings:GetTotalExpectedValue(session)

    -- Loot 5 Linen Cloth
    local ok1 = pH_Events:InjectLootItem(itemID, 5)
    if not ok1 then
        return {name = testName, passed = false, message = "Failed to inject loot"}
    end

    -- Vendor 5 Linen Cloth
    local ok2 = pH_Events:InjectVendorSale(itemID, 5)
    if not ok2 then
        return {name = testName, passed = false, message = "Failed to inject vendor sale"}
    end

    local finalNetWorth = pH_Ledger:GetBalance(session, "Assets:Cash") +
                          pH_Holdings:GetTotalExpectedValue(session)
...
function pH_Debug:Test_VendorSale_PreSession()
...
function pH_Debug:Test_PickpocketStats()
...
function pH_Debug:Test_SessionDurationAccumulator()
...
function pH_Debug:Test_GatheringNodes()
    ...
    local initialGatherInv = pH_Ledger:GetBalance(session, "Assets:Inventory:Gathering")
    local initialGatherIncome = pH_Ledger:GetBalance(session, "Income:ItemsLooted:Gathering")

    -- Inject nodes for each gathering type with realistic copper values per node
    local ok1, _ = pH_Events:InjectGatherNode("Copper Vein", 3, 8000)               -- Mining: 80s/node
    local ok2, _ = pH_Events:InjectGatherNode("Peacebloom", 5, 5000)                -- Herbalism: 50s/node
    local ok3, _ = pH_Events:InjectGatherNode("Light Leather", 2, 3000)             -- Skinning: 30s/node
    local ok4, _ = pH_Events:InjectGatherNode("Raw Brilliant Smallfish", 4, 2000)   -- Fishing: 20s/node
    ...
    local finalGatherInv = pH_Ledger:GetBalance(session, "Assets:Inventory:Gathering")
    local finalGatherIncome = pH_Ledger:GetBalance(session, "Income:ItemsLooted:Gathering")
    ...
    elseif not pass8 then
        ...
        message = string.format("OK (totalNodes=%d, 4 types, value=%s)",
            actualTotal, pH_Ledger:FormatMoney(expectedValueInjected))
    end
...
-- Log test result
function pH_Debug:LogTestResult(testName, passed, message)
...
function pH_Debug:DumpSession()
    local session = pH_SessionManager:GetActiveSession()
    if not session then
        print(COLOR_YELLOW .. "[pH Debug] No active session" .. COLOR_RESET)
        return
    end

    print(COLOR_YELLOW .. "=== pH Session Dump ===" .. COLOR_RESET)
    ...
    print(string.format("Duration: %s", pH_SessionManager:FormatDuration(time() - session.startedAt)))
    print(string.format("Zone: %s", session.zone))

    print("\nLedger Balances:")
    for acct, bal in pairs(session.ledger.balances) do
        print(string.format("  %s: %s", acct, pH_Ledger:FormatMoney(bal)))
    end

    local metrics = pH_SessionManager:GetMetrics(session)
    print("\nMetrics:")
    print(string.format("  Gold: %s", pH_Ledger:FormatMoney(metrics.cash)))
    print(string.format("  Gold/Hour: %s", pH_Ledger:FormatMoneyShort(metrics.cashPerHour)))
    ...
    if session.pickpocket and (metrics.pickpocketGold > 0 or metrics.pickpocketValue > 0 or metrics.lockboxesLooted > 0 or metrics.lockboxesOpened > 0) then
        print("\nPickpocket:")
        print(string.format("  Coin: %s", pH_Ledger:FormatMoney(metrics.pickpocketGold)))
        print(string.format("  Items Value: %s", pH_Ledger:FormatMoney(metrics.pickpocketValue)))
        ...
        if metrics.fromLockboxGold > 0 or metrics.fromLockboxValue > 0 then
            print(string.format("  From Lockboxes - Coin: %s", pH_Ledger:FormatMoney(metrics.fromLockboxGold)))
            print(string.format("  From Lockboxes - Items Value: %s", pH_Ledger:FormatMoney(metrics.fromLockboxValue)))
        end
    end
...
function pH_Debug:ShowLedger()
    ...
    if #assets > 0 then
        ...
        for _, item in ipairs(assets) do
            print(string.format("  %s: %s", item.acct, pH_Ledger:FormatMoney(item.bal)))
            totalAssets = totalAssets + item.bal
        end
        print(string.format("  " .. COLOR_GREEN .. "Total Assets: %s" .. COLOR_RESET, pH_Ledger:FormatMoney(totalAssets)))
        print("")
    end
    ...
    if #income > 0 then
        ...
        print(string.format("  " .. COLOR_GREEN .. "Total Income: %s" .. COLOR_RESET, pH_Ledger:FormatMoney(totalIncome)))
        print("")
    end
    ...
    if #expenses > 0 then
        ...
        print(string.format("  " .. COLOR_RED .. "Total Expenses: %s" .. COLOR_RESET, pH_Ledger:FormatMoney(totalExpenses)))
        print("")
    end
    ...
    if #equity > 0 then
        ...
        print(string.format("  " .. COLOR_YELLOW .. "Total Equity: %s" .. COLOR_RESET, pH_Ledger:FormatMoney(totalEquity)))
        print("")
    end
...
function pH_Debug:ShowHoldings()
    local session = pH_SessionManager:GetActiveSession()
    if not session then
        print(COLOR_YELLOW .. "[pH Debug] No active session" .. COLOR_RESET)
        return
    ...
    for itemID, holding in pairs(session.holdings) do
        ...
        for i, lot in ipairs(holding.lots) do
            print(string.format("    Lot #%d: count=%d, expectedEach=%d (%s), bucket=%s",
                i, lot.count, lot.expectedEach, pH_Ledger:FormatMoney(lot.expectedEach), lot.bucket))
        end
    end
...
function pH_Debug:ShowPriceSources()
    print(COLOR_YELLOW .. "=== Price Sources ===" .. COLOR_RESET)

    local sources = pH_PriceSources:GetAvailableSources()
...
function pH_Debug:ShowPickpocket()
    local session = pH_SessionManager:GetActiveSession()
    if not session then
        print(COLOR_YELLOW .. "[pH Debug] No active session" .. COLOR_RESET)
        return
    ...
    print(string.format("  Total Pickpocket Value: %s", 
        pH_Ledger:FormatMoney(metrics.pickpocketGold + metrics.pickpocketValue)))
    ...
    if metrics.fromLockboxGold > 0 or metrics.fromLockboxValue > 0 then
        print(string.format("\n  From Lockboxes:"))
        print(string.format("    Coin: %s", pH_Ledger:FormatMoney(metrics.fromLockboxGold)))
        print(string.format("    Items Value: %s", pH_Ledger:FormatMoney(metrics.fromLockboxValue)))
        print(string.format("    Total: %s", 
            pH_Ledger:FormatMoney(metrics.fromLockboxGold + metrics.fromLockboxValue)))
    end

    -- Ledger balances (for verification)
    print(string.format("\n  Ledger Balances (reporting only):"))
    print(string.format("    Income:Pickpocket:Coin: %s", 
        pH_Ledger:FormatMoney(metrics.incomePickpocketCoin)))
    print(string.format("    Income:Pickpocket:Items: %s", 
        pH_Ledger:FormatMoney(metrics.incomePickpocketItems)))
    if metrics.incomePickpocketFromLockboxCoin > 0 or metrics.incomePickpocketFromLockboxItems > 0 then
        print(string.format("    Income:Pickpocket:FromLockbox:Coin: %s", 
            pH_Ledger:FormatMoney(metrics.incomePickpocketFromLockboxCoin)))
        print(string.format("    Income:Pickpocket:FromLockbox:Items: %s", 
            pH_Ledger:FormatMoney(metrics.incomePickpocketFromLockboxItems)))
    end
...
function pH_Debug:ShowGathering()
    local session = pH_SessionManager:GetActiveSession()
    if not session then
        print(COLOR_YELLOW .. "[pH Debug] No active session" .. COLOR_RESET)
        return
    ...
    local gatherInv = pH_Ledger:GetBalance(session, "Assets:Inventory:Gathering")
    local gatherIncome = pH_Ledger:GetBalance(session, "Income:ItemsLooted:Gathering")
    if gatherInv > 0 or gatherIncome > 0 then
        print(string.format("\n  Inventory Value: %s", pH_Ledger:FormatMoney(gatherInv)))
        print(string.format("  Income Booked:   %s", pH_Ledger:FormatMoney(gatherIncome)))
    end
...
-- Usage: /script pH_Debug:TestHUD()
function pH_Debug:TestHUD()
    print(COLOR_YELLOW .. "=== HUD Test Data ===" .. COLOR_RESET)

    -- Ensure session is active
    if not pH_SessionManager:GetActiveSession() then
        local ok, msg = pH_SessionManager:StartSession()
        if not ok then
    ...
    -- Inject looted gold
    pH_Events:InjectLootedCoin(goldAmount)
    print(string.format("  Injected gold: %s", pH_Ledger:FormatMoney(goldAmount)))

    -- Inject inventory items (vendor trash bucket) - direct ledger posting
    pH_Ledger:Post(session, "Assets:Inventory:VendorTrash", "Income:ItemsLooted:VendorTrash", inventoryAmount)
    print(string.format("  Injected inventory (vendor trash): %s", pH_Ledger:FormatMoney(inventoryAmount)))

    -- Inject gathering nodes with value (all 4 types)
    -- Value per node simulates average loot yield from each node type
    pH_Events:InjectGatherNode("Copper Vein", 8, 8000)            -- Mining: ~80s/node
    pH_Events:InjectGatherNode("Tin Vein", 4, 12000)              -- Mining: ~1g20s/node
    pH_Events:InjectGatherNode("Peacebloom", 12, 5000)            -- Herbalism: ~50s/node
    pH_Events:InjectGatherNode("Silverleaf", 6, 4000)             -- Herbalism: ~40s/node
    pH_Events:InjectGatherNode("Light Leather", 5, 3000)          -- Skinning: ~30s/node
    pH_Events:InjectGatherNode("Raw Brilliant Smallfish", 3, 2000) -- Fishing: ~20s/node
    -- Total gathering value: 8*8000 + 4*12000 + 12*5000 + 6*4000 + 5*3000 + 3*2000 = 217000 copper (~21g70s)
    print(string.format("  Injected gathering: 38 nodes, %s total value", pH_Ledger:FormatMoney(217000)))

    -- Inject repair expense
    pH_Events:InjectRepair(expenseAmount)
    print(string.format("  Injected expenses: %s", pH_Ledger:FormatMoney(expenseAmount)))
    ...
    print(string.format("  Gold: %s", pH_Ledger:FormatMoney(expectedGold)))
    print(string.format("  Inventory: %s", pH_Ledger:FormatMoney(expectedInventory)))
    print(string.format("  Gathering: %s (38 nodes)", pH_Ledger:FormatMoney(expectedGathering)))
    print(string.format("  Expenses: -%s", pH_Ledger:FormatMoney(expenseAmount)))
    print(string.format("  Total: %s", pH_Ledger:FormatMoney(expectedTotal)))
...
-- Usage: /script pH_Debug:ResetTestHUD()
function pH_Debug:ResetTestHUD()
    -- Stop current session if active
    if pH_SessionManager:GetActiveSession() then
        pH_SessionManager:StopSession()
        print(COLOR_YELLOW .. "[pH] Test session stopped" .. COLOR_RESET)
    end

    -- Start fresh session
    pH_SessionManager:StartSession()
    pH_HUD:Update()
    print(COLOR_GREEN .. "[pH] Fresh session started for testing" .. COLOR_RESET)
end
...
local function BuildSessionSignature(session)
    ...
    local cash = pH_Ledger:GetBalance(session, "Assets:Cash") or 0
...
function pH_Debug:ScanForDuplicates()
    if not pH_DB_Account or not pH_DB_Account.sessions then
        return {
            duplicateGroups = {},
            totalDuplicates = 0,
            totalUnique = 0,
    ...
    print(COLOR_YELLOW .. "[pH] Scanning for duplicate sessions..." .. COLOR_RESET)
    ...
    for sessionId, session in pairs(pH_DB_Account.sessions) do
        ...
        if not pH_DB_Account.activeSession or pH_DB_Account.activeSession.id ~= sessionId then
    ...
    for sig, sessionIds in pairs(sessionsBySig) do
        if #sessionIds > 1 then
            ...
            local canonicalSession = pH_DB_Account.sessions[canonicalId]
...
function pH_Debug:ShowDuplicates()
...
function pH_Debug:PurgeDuplicates(confirm)
    -- First scan for duplicates
    local result = self:ScanForDuplicates()

    if result.totalDuplicates == 0 then
        print(COLOR_GREEN .. "[pH] No duplicates found. Database is clean." .. COLOR_RESET)
        return
    end
    ...
    if not confirm then
        ...
        print(COLOR_YELLOW .. "  Location: WTF/Account/<YourAccount>/SavedVariables/pH.lua" .. COLOR_RESET)
        print("")
    ...
    for _, group in ipairs(result.duplicateGroups) do
        for _, duplicateId in ipairs(group.duplicate_ids) do
            if pH_DB_Account.sessions[duplicateId] then
                pH_DB_Account.sessions[duplicateId] = nil
